...
impl HeaderParser for DefaultHeaderParser {
    #[cfg(not(target_arch = "aarch64"))]
    type Serializer = StandardHeaderParser;

    #[cfg(target_arch = "aarch64")]
    type Serializer = Aarch64NeonHeaderParser;
    
    #[cfg(not(feature = "simd"))]
    type Deserializer = StandardHeaderParser;
    #[cfg(feature = "simd")]
    type Deserializer = OptimizedHeaderParser;
}
...
pub(crate) mod tests {
    ...
    const HEADER_BYTES: [u8; HEADER_SIZE] = [
        6,
        0,
        9,
        0,
    ...
    pub(crate) fn test_deserializer<D: HeaderDeserializer>() {
        ...
        assert_eq!(header.flags(), MessageFlags::COMPRESSED | MessageFlags::HAS_PAYLOAD);
